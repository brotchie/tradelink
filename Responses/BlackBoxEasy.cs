...
namespace Responses
{
    ...
    public class BlackResponseEasy : DayTradeResponse
    {
        ...
        protected bool newTrade() { return (Pos.Size != 0) && (Pos.AvgPrice== 0); }
        /// <summary>
        ...
        protected decimal Profit { get { return (Pos.Size> 0) ? tick.trade - Pos.AvgPrice: Pos.AvgPrice- tick.trade; } }
        void checkMoveStop()
        {
            if (Math.Abs(sPrice - this.tick.trade) > (this.stop * 1.5m)) sPrice = tick.trade - ((stop * 1.5m) * ((Pos.Size < 0) ? -1 : 1));
        }
        bool hitStop() { return (((tick.trade - sPrice) * ((Pos.Size< 0) ? 1 : -1))<=0); }
        bool hitProfit() { return (ProfitSize>0) && (ProfitTarget>0) && (Profit > ProfitTarget) && (Pos.Size== EntrySize); }
        void getStop() { if (Stop!=0) sPrice = Pos.AvgPrice - (stop * ((Pos.Size>0) ? 1 : -1)); }
        bool Enter()
        {
            Side = true;
        ...
        protected override Order ReadOrder(Tick t,BarList barlist)
        {
            ...
            int adjust = 0;
            if (newTrade()) getStop(); 
            else if (Pos.Size != 0) // if we have entry price, check exits
            {
                if (!IgnoreTick()) checkMoveStop();
                if (!IgnoreTick() && this.Exit()) adjust = Pos.Size* -1;
                else if (this.hitProfit()) adjust = (Pos.Size> 0) ? ProfitSize : ProfitSize * -1;
                else if (this.hitStop()) adjust = Pos.Size* -1;
            }
            else if (IgnoreTick()) adjust = 0;
            else if (this.Enter()) // if haven't entered, check entry criteria
                adjust = EntrySize * (Side ? 1 : -1);
            MarketOrder o = new MarketOrder(Symbol, adjust);
            return o;
        }
    }
}
